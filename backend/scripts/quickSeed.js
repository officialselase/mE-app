...
try {
  ...
  const insertProject = db.prepare('INSERT INTO projects (title, description, images, technologies, github_url, live_url, featured, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime("now"), datetime("now"))');
  ...
  const insertThought = db.prepare('INSERT INTO thoughts (title, snippet, content, date, featured, tags, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"), datetime("now"))');
  ...
  const insertWork = db.prepare('INSERT INTO work_experience (company, position, description, start_date, end_date, current, technologies, display_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime("now"), datetime("now"))');
  ...
  console.log('Thoughts count:', db.prepare('SELECT COUNT(*) as count FROM thoughts').get());
  console.log('Work count:', db.prepare('SELECT COUNT(*) as count FROM work_experience').get());
